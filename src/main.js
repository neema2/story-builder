...
import { setApiKey, getApiKey, validateApiKey, generateStoryBrief, generateWholeStory, generateCharacterSheet, generateIllustration, getLastStoryBrief } from './api.js';
...
const state = {
  ...
  characterSheet: '',                   // locked-in character visual descriptions
  referenceImage: null,                 // page 1 image used as reference for subsequent pages
};
...
async function startStoryGeneration() {
  try {
    ...
    // Pass 2.5: Generate character reference sheet for visual consistency
    ...
    // Generate remaining illustrations sequentially (each sees the previous)
    // This runs in background while user reads page 1
    generateRemainingIllustrations(1);
...
async function generateIllustrationForPage(pageIndex) {
  ...
  try {
    const imageUrl = await generateIllustration({
      ...
      referenceImageBase64: pageIndex > 0 ? state.referenceImage : null,
    });

    if (imageUrl) {
      state.pages[pageIndex].imageUrl = imageUrl;

      // Always update reference to the latest generated image (sequential chain)
      state.referenceImage = imageUrl;
      console.log(`📌 Page ${pageIndex + 1} image stored as reference for next page`);
...
// Generate illustrations sequentially so each page sees the previous page's image
async function generateRemainingIllustrations(startIndex) {
  for (let i = startIndex; i < state.pages.length; i++) {
    if (state.pages[i].imageUrl) continue; // already generated
    await generateIllustrationForPage(i);
  }
}
  ...
  if (nextPageBtn) {
    nextPageBtn.addEventListener('click', async (e) => {
      ...
      if (!state.pages[state.currentPage]?.imageUrl) {
        // Image not ready yet — show text with emoji placeholder, await image
        render();
        await generateIllustrationForPage(state.currentPage);
      } else {
        // Image already generated by the sequential chain
        render();
      }
    });
  }
